...
Update time: 2022-07-21 18:41:23.
'''
...
class DeconvBlock(nn.Module):
    def __init__(self, c_in, c_out, kernel_size, stride, padding, norm, use_act):
        ...
        layers = [
                ...
                ConvBlock(c_in, c_out, kernel_size=kernel_size, stride=1,
                    padding=padding, norm=norm, use_act=use_act),
                ]
...
class Generator(nn.Module):
    def __init__(self, c_in, n_features=64, n_resblocks=9, n_strides=3, dropout=0.5):
        ...
        self.head = ConvBlock(c_in, n_features, kernel_size=7, stride=1, padding=3, norm='bn', use_act=True)
        ...
        for ii in range(n_strides):
            layers.append(ConvBlock(cii, cii2, kernel_size=3, stride=2, padding=1, norm='bn', use_act=True))
        ...
        for ii in range(n_resblocks):
            layers.append(ResBlock(c0, norm='bn'))
            layers.append(nn.ReLU())
        ...
        for ii in range(n_strides):
            ...
            layers.append(DeconvBlock(cii, cii2, kernel_size=3, stride=2, padding=1, norm='bn', use_act=True))

        self.decoder = nn.Sequential(*layers)

        self.tail = nn.Sequential(
                ConvBlock(cii2, c_in, kernel_size=3, stride=1, padding=1, norm=False, use_act=False),
                nn.Tanh())
...
class Discriminator(nn.Module):
    def __init__(self, c_in, dropout):
        ...
        self.layers = nn.Sequential(
                nn.Conv2d(c_in, 32, kernel_size=4, stride=2, padding=1, padding_mode='reflect'),
                nn.LeakyReLU(0.2, True),
                ConvBlock(32, 64, kernel_size=4, stride=2, padding=1, norm='bn', use_act=True),
                nn.Dropout2d(dropout),
                ConvBlock(64, 128, kernel_size=4, stride=2, padding=1, norm='bn', use_act=True),
                nn.Dropout2d(dropout),
                ConvBlock(128, 1, kernel_size=4, stride=2, padding=1, norm=False, use_act=False),
                )
